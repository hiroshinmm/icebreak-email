...
- **画像抽出**: 記事の OGP イメージ、RSS 内のメディアタグ、または記事本文内の画像から、代表的な画像を自動で抽出する（YouTubeサムネイルやLazy Loading画像の対応、相対パスの自動解決を含む）。さらにCloudflareのようなボット保護が稼働しているサイトからアクセス拒否（403/503）を受けた場合でも、Puppeteer（ヘッドレスブラウザ）を自動起動してフォールバック取得する堅牢な仕組みを持つ。
